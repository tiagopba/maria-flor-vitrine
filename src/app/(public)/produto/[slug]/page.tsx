...
import { ProductColorSwatches } from "@/components/catalog/ProductColorSwatches";
...
import { listActiveColorsPublic } from "@/lib/db/colors";
import { getProductBySlugPublic, getPublishedGroupSiblings, getRelatedProductsPublic } from "@/lib/db/products";
...
export default async function ProductPage({ params }: PageProps<"/produto/[slug]">) {
  ...
  const [sellers, related, categories, paymentSettings, groupSiblings, colors] = await Promise.all([
    ...
    product.product_group_id ? listActiveColorsPublic() : Promise.resolve([]),
  ]);
  ...
  const currentColorName = product.color_id ? (colors.find((c) => c.id === product.color_id)?.name ?? null) : null;
      ...
      <div className="grid gap-8 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          ...
          <ProductColorSwatches
            currentColorId={product.color_id}
